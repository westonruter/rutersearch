...
HTMLCODE
	...
	#Parse it, add it, and display it- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	   ...
	   #Get the base href -  -  -  -  -  -  -  -  -  -  -  -
		...
		#prepare to add entries to MySQL database (if selected)
		my $insertEntry = $database->prepare('INSERT INTO rutersearch VALUES (?,?,?,?,?,?,?)') if(lc $config{'dbtype'} eq 'mysql');
		...
		for($urlcontent =~ /<(?:a|area).+?href=(?:"|'|)(.+?)(?:"|'|\s|>)/img){
			...
			if(lc $config{'dbtype'} eq 'text'){	#if you are using a $config{'delimiter'} delimited text file
				...
				$insertEntry->execute(substr("$month/$dayOfMonth/$year $hour:$minute:$second", 0, 19), 'html', substr($title, 0, 128), substr($description, 0, 255), substr($url, 0, 255), substr($keywords, 0, 255), substr($author, 0, 64));
			}
